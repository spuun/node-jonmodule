...
var loadmodule = (filename, api) => {
	...
	var createModuleProcess = () => {
		...
		modulep.command = (cmd, args) => {
			modulep.send({command: cmd, data: args});
		};
	};
...
var initmodule = () => {
	...
	var handleParentCommand = (command, data) => {
		if (modInstance.hasOwnProperty(command)) {
			return modInstance[command].apply(modInstance, data);
		}
	};
	...
	process.on('message', (msg) => {
		...
		if (msg.command) {
			handleParentCommand(msg.command, msg.data);
		}
	});
};
